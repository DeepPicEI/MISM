...
class Decoder(BaseModule):
    ...
    def forward(self, t_in, x, hw_shape):
        t_in = rearrange(t_in, 'b c h w ->b (h w) c')
...
class UMAEFPNNeck(BaseModule):
    ...
    def forward(self, encoder_outs, x,  hw_shape):
        x = rearrange(x, 'b c h w ->b (h w) c')
        for i in range(len(self.Decoder)):
            x, hw_shape, out_hw_shape = self.Decoder[i](encoder_outs[-i - 2], x, hw_shape)
