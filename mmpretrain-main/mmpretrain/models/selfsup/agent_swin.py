...
class AgentAttention(nn.Module):
    ...
    def forward(self, x, mask=None, hw_shape=None):
        ...
        q, k, v, a = qkv[0], qkv[1], qkv[2], qkv[3]  # make torchscript happy (cannot use tensor as tuple)
        # q, k, v: b, n, c

        downstream_agent_num = self.downstream_agent_shape[0] * self.downstream_agent_shape[1]
        agent_tokens = self.pool(a.reshape(b, h, w, c).permute(0, 3, 1, 2)).reshape(b, c, -1).permute(0, 2, 1)
...
@MODELS.register_module()
class PreAgent(BaseSelfSupervisor):
    """SimMIM.
