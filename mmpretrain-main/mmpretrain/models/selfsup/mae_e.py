...
class Encoder(BaseModule):
    ...
    def forward(self, x, idx, hw_shape, ids_restore):
        if idx is not None:
            t_out = self.select(x, idx)
            ...
            for block in self.blocks:
                t_out = block(t_out)
        x_down, down_hw_shape = self.downsample(t_out, hw_shape)
        return x_down, down_hw_shape, t_out, hw_shape


@MODELS.register_module()
class UMAEencorder(BaseModule):
    def __init__(self,
    ...
    def forward_mask(self, x):
        ...
        for i in range(len(self.Encoder)):
            x, hw_shape, out, out_hw_shape = self.Encoder[i](x, idx[i], hw_shape,ids_restore=idx_sequence[i])
        ...
        for i in self.out_indices:
            ...
            if i == self.out_indices[-1]:
                ...
                norm_out = norm_layer(encoder_outs[i])
                norm_outs.append(norm_out)

        return norm_outs, x, hw_shape, idx_sequence, mask[0]
...
@MODELS.register_module()
class UMAEBN(BaseSelfSupervisor):
    """MAE.
    ...
    def loss(self, inputs: torch.Tensor, data_samples: List[DataSample],
        ...
        encoder_outs, x, hw_shape, idx_sequence, mask = self.backbone(inputs)
        pred = self.neck(encoder_outs, x, hw_shape, idx_sequence)
