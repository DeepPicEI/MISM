...
from mmcv.cnn import build_norm_layer, DepthwiseSeparableConvModule
...
class Encoder(BaseModule):
    ...
    def forward(self, x, idx, hw_shape, idx_sequence):
        if idx is not None:
            t_in = self.select(x, idx)
        else:
            t_in = x
        for block in self.blocks:
            t_out = block(t_in)
            t_out = self.reproduce(t_out, idx_sequence)
...
class BasicConvBlock(BaseModule):
    ...
    def __init__(self,
        ...
        down_sample = []
        down_sample.append(nn.Conv2d(in_channels=in_channels, out_channels=in_channels*2,
                                     kernel_size=3, padding=1))
        down_sample.append(nn.MaxPool2d(kernel_size=2, stride=2))
    ...
    def forward(self, x, t_in):
        ...
        add = t_in + conv_out
        out = self.down_sample(add)
        return out, add


@MODELS.register_module()
class UMAVITFPN(BaseModule):
    def __init__(self,
        ...
        self.post_token = nn.Parameter(torch.zeros(1, embed_dims, 1, 1))
        ...
        self.Down_net = ModuleList()

        in_channels = embed_dims
        for i in range(len(depths)):
            self.Down_net.append(BasicConvBlock(
                in_channels=in_channels,
                num_convs=2,
                stride=1,
                dilation=1,
    ...
    def forward_mask(self, x):
        ...
        post_tokens = self.post_token.repeat(
            x.shape[0], 1, hw_shape[0], hw_shape[1])
        ...
        for i in range(len(self.Encoder)):
            x, hw_shape, out, out_hw_shape = self.Encoder[i](x, idx[i], hw_shape, idx_sequence[i])
            h, w = out_hw_shape
            encoder_outs.append(rearrange(out, 'b (h w) c -> b c h w', h=h, w=w))
        x = self.Bottle(x, idx=idx[-1], ids_restore=idx_sequence[-1])
        H, W = hw_shape
        encoder_outs.append(rearrange(x, 'b (h w) c -> b c h w', h=H, w=W))

        output = []
        for i, layer in enumerate(self.Down_net):
            post_tokens, out = layer(post_tokens, encoder_outs[i])
            output.append(out)
        return output, output[-1], hw_shape, mask[0]
...
@MODELS.register_module()
class UMAEFPN(BaseSelfSupervisor):
    """MAE.
    ...
    def loss(self, inputs: torch.Tensor, data_samples: List[DataSample],
        ...
        encoder_outs, x, hw_shape, mask = self.backbone(inputs)
        pred = self.neck(encoder_outs, x, hw_shape)
