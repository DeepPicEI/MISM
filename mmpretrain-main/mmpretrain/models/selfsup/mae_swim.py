...
class SwinBlockSequence(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(depth):
            conv = BasicConvBlock(in_channels=embed_dims,
                                  out_channels=embed_dims)
            self.blocks_conv.append(conv)
    ...
    def forward(self, x, hw_shape):
        ...
        x2 = rearrange(x, 'b (h w) c -> b c h w', h=hw_shape[0], w=hw_shape[1])
        for block in self.blocks_conv:
            x2 = block(x2)
        x2 = rearrange(x2, 'b c h w ->b (h w) c')
        x_out = x1+x2
        if self.downsample:
            x_down, down_hw_shape = self.downsample(x_out, hw_shape)
            return x_down, down_hw_shape, x_out, hw_shape
        else:
            return x_out, hw_shape, x_out, hw_shape


@MODELS.register_module()
class UMAESwin(BaseModule):
...
@MODELS.register_module()
class UMAESimMIM(BaseSelfSupervisor):
    """SimMIM.
